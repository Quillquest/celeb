...
@section('content')
    ...
    <!-- Hero Section -->
    <section
        class="relative pt-24 pb-16 bg-gradient-to-br from-gray-900 via-{{ $formConfig['primaryColor'] ?? 'primary' }}-900 to-gray-900 overflow-hidden">
        <!-- Background Elements -->
        <div class="absolute inset-0 overflow-hidden">
            <div
                class="absolute -top-20 -right-20 w-96 h-96 bg-{{ $formConfig['primaryColor'] ?? 'primary' }}-400 rounded-full mix-blend-overlay filter blur-3xl opacity-20 animate-float">
            </div>
            <div
                class="absolute -bottom-20 -left-20 w-96 h-96 bg-{{ $bookingType == 'donation' ? 'accent' : $formConfig['primaryColor'] ?? 'primary' }}-500 rounded-full mix-blend-overlay filter blur-3xl opacity-20 animate-float animation-delay-2000">
            </div>
                ...
                <div class="text-center md:text-left">
                    <div class="flex flex-col md:flex-row md:items-center gap-3 mb-3">
                        <div class="flex items-center gap-3">
                            <div
                                class="w-12 h-12 rounded-full bg-{{ $formConfig['primaryColor'] ?? 'primary' }}-500/20 border border-{{ $formConfig['primaryColor'] ?? 'primary' }}-500/30 flex items-center justify-center">
                                <i data-lucide="{{ $formConfig['icon'] ?? 'calendar' }}"
                                    class="w-6 h-6 text-{{ $formConfig['primaryColor'] ?? 'primary' }}-300"></i>
                            </div>
                            <div>
                                <h1 class="text-3xl md:text-4xl lg:text-5xl font-bold text-white">
                                    @if ($bookingType == 'donation')
                                        <span class="text-accent-400">Donate</span> with {{ $celebrity->name }}
                                    @elseif($bookingType == 'fan_card')
                                        <span class="text-primary-400">VIP Fan Card</span> - {{ $celebrity->name }}
                                    @else
                                        <span class="text-primary-400">Book</span> {{ $celebrity->name }}
                                    @endif
                                </h1>
                                <p class="text-lg text-white/80 mt-2">
                                    {{ $formConfig['subtitle'] ?? 'Professional booking service' }}</p>
                            </div>
                        </div>

                        @if (isset($celebrity->featured) && $celebrity->featured == 'featured')
                            <span
                    ...
                    <div class="flex flex-wrap gap-3 justify-center md:justify-start mb-6">
                        ...
                        @if (isset($celebrity->years_active))
                            <div
                                class="inline-flex items-center px-3 py-1.5 bg-white/10 backdrop-blur-sm rounded-lg text-white text-sm">
                                <i data-lucide="calendar" class="w-4 h-4 mr-2"></i>
                                {{ $celebrity->years_active }} Years Active
                            </div>
                        @endif
                    ...
                    <p class="text-white/80 text-lg max-w-2xl mb-6">
                        {{ $formConfig['description'] ?? 'Book ' . $celebrity->name . ' for your next event and create unforgettable memories with one of the most sought-after talents in the industry.' }}
                    </p>

                    @if (isset($celebrity->booking_fee))
                        <div
                            class="inline-flex items-center px-4 py-2 bg-{{ $formConfig['primaryColor'] ?? 'primary' }}-500/20 border border-{{ $formConfig['primaryColor'] ?? 'primary' }}-500/30 rounded-lg text-{{ $formConfig['primaryColor'] ?? 'primary' }}-300">
                            <i data-lucide="tag" class="w-5 h-5 mr-2"></i>
                            <span class="text-lg font-semibold">
                                @if ($bookingType == 'donation')
                                    Suggested donation:
                                    ${{ is_numeric($celebrity->booking_fee) ? number_format((float) $celebrity->booking_fee / 10, 0) : '50' }}+
                                @elseif($bookingType == 'fan_card')
                                    Fan Card:
                                    ${{ is_numeric($celebrity->booking_fee) ? number_format((float) $celebrity->booking_fee / 5, 0) : '200' }}
                                @else
                                    ${{ is_numeric($celebrity->booking_fee) ? number_format((float) $celebrity->booking_fee, 0) : $celebrity->booking_fee }}
                                @endif
                            </span>
                            <span class="ml-1 text-{{ $formConfig['primaryColor'] ?? 'primary' }}-300/80">
                                @if ($bookingType == 'donation')
                                    minimum
                                @elseif($bookingType == 'fan_card')
                                    annual membership
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            <div class="max-w-4xl mx-auto">
                ...
                <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden"
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <h2 class="text-xl font-bold text-gray-900 dark:text-white">
                                        {{ $formConfig['title'] ?? 'Book Celebrity' }}</h2>
                                    <p class="text-sm text-gray-600 dark:text-gray-400">
                                        {{ $formConfig['subtitle'] ?? 'Complete the form below' }}</p>
                                </div>
                            </div>
                            @if ($bookingType == 'donation')
                    ...
                    <!-- Form Content -->
                    <div class="p-6">
                        <form action="{{ route('processBooking') }}" method="POST" class="space-y-8" id="booking-form"
                            x-ref="bookingForm">
                            @csrf
                            <input type="hidden" name="celebrity_id" value="{{ $celebrity->id }}">
                            <input type="hidden" name="booking_type" value="{{ $bookingType }}">
                            <input type="hidden" name="amount"
                                value="{{ is_numeric($celebrity->booking_fee) ? (float) $celebrity->booking_fee : 1000 }}">
                                ...
                                <div x-show="tab === 'details'" x-cloak class="space-y-6">
                                    <div
                                        ...
                                        <p class="text-red-700 dark:text-red-300">Your donation will support
                                            {{ $celebrity->name }}'s chosen charitable causes and make a real difference in
                                            the world.</p>
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div class="md:col-span-2">
                                            ...
                                            <textarea id="personal_message" name="personal_message" rows="3"
                                                placeholder="Share why you're making this donation or a message for {{ $celebrity->name }}..."
                                ...
                                <div x-show="tab === 'charity'" class="space-y-6" x-cloak>
                                    <div
                                        ...
                                        <h3
                                            ...
                                            {{ $celebrity->name }}'s Supported Charities
                                        </h3>
                                        <div class="space-y-3">
                                            <label
                                    ...
                                    <div
                                        ...
                                        <div class="flex items-center mb-2">
                                            ...
                                            <h3 class="text-lg font-semibold text-primary-900 dark:text-primary-100">
                                                {{ $celebrity->name }} VIP Fan Card</h3>
                                        </div>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div>
                                            ...
                                            <input type="date" id="event_date" name="event_date" required
                                                data-label="Event Date" min="{{ date('Y-m-d', strtotime('+1 day')) }}"
                                    ...
                                    <div
                                        ...
                                        <ul class="space-y-2 pl-8 list-disc">
                                            ...
                                            <li class="text-gray-600 dark:text-gray-400">Transportation from local
                                                airport/hotel</li>
                                            <li class="text-gray-600 dark:text-gray-400">{{ $celebrity->name }} requires
                            ...
                            <div x-show="tab === 'payment'" class="space-y-6" x-cloak>
                                ...
                                <div
                                    ...
                                    <h3 class="text-xl font-semibold text-gray-900 dark:text-white mb-4 flex items-center">
                                        <i data-lucide="{{ $formConfig['icon'] ?? 'calendar' }}"
                                            class="w-6 h-6 text-{{ $formConfig['primaryColor'] ?? 'primary' }}-600 mr-2"></i>
                                        @if ($bookingType == 'donation')
                                            Donation Summary
                                        @elseif($bookingType == 'fan_card')
                                            Membership Summary
                                    ...
                                    <div class="space-y-3">
                                        <div class="flex justify-between">
                                            ...
                                            <span
                                                class="font-medium text-gray-900 dark:text-white">{{ $celebrity->name }}</span>
                                        </div>
                                        @if ($bookingType == 'donation')
                                            <div class="flex justify-between">
                                        ...
                                        @else
                                            <div class="flex justify-between">
                                                ...
                                                <span
                                                    class="font-medium text-gray-900 dark:text-white">${{ is_numeric($celebrity->booking_fee) ? number_format((float) $celebrity->booking_fee, 2) : $celebrity->booking_fee }}</span>
                                            </div>
                                            <div class="flex justify-between">
                                                <span class="text-gray-600 dark:text-gray-400">Service Fee:</span>
                                                <span
                                                    class="font-medium text-gray-900 dark:text-white">${{ is_numeric($celebrity->booking_fee) ? number_format((float) $celebrity->booking_fee * 0.05, 2) : '50.00' }}</span>
                                            </div>
                                            <div
                                                class="border-t border-gray-200 dark:border-gray-700 pt-3 flex justify-between">
                                                <span class="font-semibold text-gray-900 dark:text-white">Total
                                                    Amount:</span>
                                                <span class="font-bold text-lg text-primary-600">
                                                    ${{ is_numeric($celebrity->booking_fee) ? number_format((float) $celebrity->booking_fee * 1.05, 2) : number_format(1050, 2) }}
                                                </span>
                                            </div>
                                        @endif
                                    </div>
                                ...
                                <div class="flex items-start">
                                    <div class="flex items-center h-5">
                                        <input id="terms" name="terms" type="checkbox" required
                                            class="w-4 h-4 text-{{ $formConfig['primaryColor'] ?? 'primary' }}-600 bg-gray-100 dark:bg-gray-800 rounded border-gray-300 dark:border-gray-700 focus:ring-{{ $formConfig['primaryColor'] ?? 'primary' }}-500">
                                    </div>
                                    <label for="terms" class="ml-3 text-sm text-gray-600 dark:text-gray-400">
                                        I agree to the <a href="#"
                                            class="text-{{ $formConfig['primaryColor'] ?? 'primary' }}-600 hover:underline">Terms
                                            and Conditions</a> and
                                        @if ($bookingType == 'donation')
                                            <a href="#"
                                                class="text-{{ $formConfig['primaryColor'] ?? 'primary' }}-600 hover:underline">Donation
                                                Policy</a>
                                        @elseif($bookingType == 'fan_card')
                                            <a href="#"
                                                class="text-{{ $formConfig['primaryColor'] ?? 'primary' }}-600 hover:underline">Membership
                                                Agreement</a>
                                        @else
                                            <a href="#"
                                                class="text-{{ $formConfig['primaryColor'] ?? 'primary' }}-600 hover:underline">Cancellation
                                                Policy</a>
                                        @endif
                                    </label>
                                </div>
                                ...
                                <div class="flex justify-between">
                                    <button type="button"
                                        @click="tab = '{{ $bookingType == 'donation' ? 'charity' : ($bookingType == 'fan_card' ? 'benefits' : 'requirements') }}'"
                                        class="inline-flex items-center px-6 py-3 bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-800 dark:text-white font-medium rounded-lg transition-colors">
                                        <i data-lucide="arrow-left" class="w-4 h-4 mr-2"></i>
                                        Back
                                    </button>
                                    <button type="button" @click="submitForm()" id="complete-booking-btn"
                                        ...
                                        'hover:bg-{{ $formConfig['primaryColor'] ?? 'primary' }}-700'"
                                        class="inline-flex items-center px-8 py-3 bg-{{ $formConfig['primaryColor'] ?? 'primary' }}-600 text-white font-medium rounded-lg transition-colors">
                                        <i data-lucide="{{ $formConfig['icon'] ?? 'check-circle' }}"
                                            class="w-5 h-5 mr-2"></i>
                                        {{ $formConfig['buttonText'] ?? 'Complete Booking' }}
                                    </button>
                                </div>
                            </div>
                        </form>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const form = document.querySelector('form[action="{{ route('processBooking') }}"]');
